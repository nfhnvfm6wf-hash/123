(() => {
  ...
  /* ---------------------------------------------------------------------
     Real photo slots (.js-photo): перебор форматов (png/jpg/jpeg/webp)
     обрабатывается инлайновым onerror прямо в HTML (см. index.html) —
     это надёжнее, чем внешний скрипт, который может подключиться уже
     после того, как картинка попыталась загрузиться. Если ни один формат
     не нашёлся, onerror добавляет класс is-broken, и вместо фото остаётся
     декоративная SVG-заглушка.
  --------------------------------------------------------------------- */
